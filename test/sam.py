...
def SAM(apple, d_point):
    ...
    rd = np.sum((r * d_point), axis=0)
    r_abs = (np.sum((r ** 2), axis=0)) ** 2 
    d_abs =(np.sum((d_point ** 2), axis=0)) ** 2
    #rd = (r * d_point).sum(axis=0) #<r,d>
    #r_abs = ((r ** 2).sum(axis=0)) ** 2 #||r||
    #d_abs =((d_point ** 2).sum(axis=0)) ** 2#||d||
    temp = rd / (r_abs * d_abs)   
